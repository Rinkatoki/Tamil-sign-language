...
def load_tamil_mapping(filepath):
    mapping = {}
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        for line in f:
            ...
            tamil_char = tamil_part.split("(")[0].strip()

            mapping[number] = tamil_char

    return mapping


TAMIL_MAP = load_tamil_mapping(MAPPING_FILE)
...
while True:
    ...
    if res.multi_hand_landmarks:
        ...
        if thumbs_down == 0:
            ...
            pred_number = classes[idx]

            # Convert numeric label → Tamil character
            pred = TAMIL_MAP.get(pred_number, "")

            # Ignore background prediction
            if pred.lower() == "background":
                continue
