...
while True:
    ...
    if res.multi_hand_landmarks:
        ...
        stable_thumbs = Counter(thumb_buffer).most_common(1)[0][0]
        now = time.time()
        # -------------------------
        # -------------------------
        # GESTURE PRIORITY (stable)
        # -------------------------
        # gesture priority logic
        if stable_thumbs >= 2:
            gesture_mode = "DELETE"

        elif stable_thumbs == 1 and has_left and has_right:
            gesture_mode = "SPACE"
